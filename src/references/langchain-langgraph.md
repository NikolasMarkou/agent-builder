...
Parameters:
- `model`: `str` (`"openai:gpt-4o"`, `"claude-sonnet-4-6"`) or `BaseChatModel` instance
...
agent = create_agent(model="claude-sonnet-4-6", tools=[...], response_format=Analysis)
...
**Model fallback:**
```python
middleware=[ModelFallbackMiddleware(fallbacks=["claude-sonnet-4-6", "openai:gpt-4o-mini"])]
```
